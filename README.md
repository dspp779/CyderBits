...
The validated path is classic 2D Win32 graphics: **DirectDraw → Wine wined3d/OpenGL** and GDI. The current packaged `CX26.3.0-W11-Cyder007` engine also contains an x86_64 **MoltenVK** runtime for Wine Vulkan (repacking defaults to `VULKAN_MODE=with` + `VULKAN_SOURCE=existing`), but BlueCG does not use Vulkan, DXVK, dxmt, or D3DMetal.
...
- **Cyder.app:** macOS 10.15+ (`LSMinimumSystemVersion`); Game Library / settings UI on **11+** (10.15 keeps only Bash file selection / launch)
