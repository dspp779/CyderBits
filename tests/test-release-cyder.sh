...
assert_eq "$(tr -d '[:space:]' <"$ROOT/config/cyder-app-version.txt")" "0.10.0" \
  "released 0.10.0 must be the current shared App version"
...
release_out="$(
  SIGN_IDENTITY='Developer ID Application: Example' \
    bash "$ROOT/scripts/release-cyder.sh" --channel release --version 0.10.0 --dry-run --out-dir /tmp/cyder-release-dry 2>&1
)"
