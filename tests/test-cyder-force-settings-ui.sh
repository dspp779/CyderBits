...
assert_contains "$library_ui" "private func detailRetinaChanged()" "game-library Retina toggle should have a dedicated DPI synchronization handler"
assert_contains "$library_ui" "let target = detailRetina.state == .on ? 192 : 96" "game-library Retina toggle should suggest 192 or 96 DPI"
assert_contains "$ui" "retina.action = #selector(retinaChanged)" "global Retina control should use the synchronization handler"
assert_contains "$library_ui" "detailRetina.action = #selector(detailRetinaChanged)" "game-library Retina control should use the synchronization handler"
assert_contains "$library_ui" "為遊戲建立獨立設定" "game library should expose independent settings"
