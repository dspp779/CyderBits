...
| `bundle-wine-dylibs.sh` | 遞迴收集 Homebrew／graphics／media dylib，處理 proxy-libintl/gettext 同名衝突並改 `@loader_path` |
| `strip-wine-install.sh` | 剝除 engine 非 runtime 與 PE DWARF debug sections；打包前 staging |
| `build-universal-zstd.sh` | 由固定 checksum 的官方 source 建 dependency-free universal zstd CLI |
...
| `build-dxvk2.sh` | 上游 DXVK 2.7.1 研究用建置腳本；目前不進 Cyder 發布流程（待開發） |
...
create-cyder-app.sh
    ...
    → install-libarchive-tar.sh, enable-mac-retina-hires.sh（bootstrap；Mono/Gecko 改由 Wine 需要時提示安裝）
...
| `tests/test-build-media-stack.sh` | 最小 GStreamer feature set 與 media dylib bundling 契約 |
...
| `tests/test-cyder-dxvk2.sh` | 待開發建置腳本的隔離性測試；不代表目前 Cyder release 支援 DXVK 2 |
...
正式打包會移除 PE `.debug_*` sections；未剝除的開發 engine 可設
`CYDER_KEEP_DEBUG_SYMBOLS=1`。量測見 [Runtime 與 Bottle 體積研究](cyder-runtime-size-study.md)，
zstd 相容範圍見 [內建 Universal zstd](universal-zstd.md)。
