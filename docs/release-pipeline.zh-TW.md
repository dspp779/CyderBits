...
| 預設 App 版本 | `0.13.0-dev`（可用 `--version` 改） | `0.13.0`（必須是乾淨 semver；來源為 `config/cyder-app-version.txt`） |
...
```bash
# 預設 adhoc、版本 0.13.0-dev、使用 pinned 或 create-cyder-app 預設引擎尋找邏輯
bash scripts/release-cyder.sh --channel test

# 指定本機 rc 引擎與版本字串
bash scripts/release-cyder.sh --channel test \
  --version 0.13.0-branch-foo \
...
```bash
SIGN_IDENTITY=- CYDER_APP_VERSION=0.13.0-dev \
