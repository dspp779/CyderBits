# BlueCG winemac runtime 比較：原版、A2、A4、A6

> 更新：2026-07-13
>
> 對象：CrossOver 26.2.0 source build、x86_64 Wine、BlueCG DirectDraw／wined3d／OpenGL 路徑。
>
> 本文件比較 baseline、A2、A4 與目前正式的 A6 runtime；A5 與 A6-R1～R5 的逐步實驗另見 [bluecg-winemac-experiments.md](bluecg-winemac-experiments.md)。

## 一覽結論

| 項目 | 原版 CX26 | A2 | A4 | **A6 final** |
|------|-----------|----|----|-----------|
| 非 Retina 進入世界後拖曳 | **黑屏** | **正常** | **正常** | **正常** |
| Retina 進入世界後拖曳 | **黑屏** | **黑屏** | **正常** | **正常** |
| Retina + launcher 啟動 | 可啟動；resize 後黑屏 | 可啟動；resize 後黑屏 | **可啟動；resize 正常** | **可啟動；resize 正常** |
| Alt+Enter（進入世界後） | 已知會黑屏 | Retina=`y` 會黑；Retina=`n` 尚未完整驗證 | **正常** | **正常** |
| 最小化／還原 | 已有黑屏／同源風險 | 未完成完整矩陣 | **正常** | **正常且尺寸固定** |
| 高 DPI 初始視窗 | backing 路徑可建立，但 resize 風險高 | Retina off 可避開黑屏，但畫質／黑邊需另測 | 不黑，但 render target 不跟著外層視窗放大，內容固定左下且上／右有黑區 | **不黑、render target 跟隨、無 A4 黑邊** |
| BlueCG 目前可用性 | 不可接受 | 非 Retina 可用 | Retina 與非 Retina 均可用，但高 DPI 畫質有折衷 | **目前正式方案** |
| 修改範圍 | 無 | Retina-only guard | 全域停用 explicit backing update/reset | **per-view defer + same-view in-place + restore rect** |
...
使用 A6 final 時，則可保留 `RetinaMode=y` 與高 DPI，並在進入世界後正常拖曳、Alt+Enter 或最小化／還原；A6 的驗收細節見 [bluecg-winemac-a6-engine.md](bluecg-winemac-a6-engine.md)。
...
A6 保留正常的 DPI backing 尺寸更新，但把同一個 view 的 resize 改成 in-place commit：live resize 期間延後 sync，結束後以最終尺寸更新既有 drawable；Alt+Enter 等同 view 的 programmatic resize 也走相同路徑。只有真正 view switch 才保留原本的 attach/reset 行為。

此外，`WINDOW_DID_UNMINIMIZE` 不再用 Retina-converted Cocoa frame 覆蓋 user32 的 saved restore rect，避免每次最小化／還原把尺寸放大。這是目前 BlueCG 同時滿足 Retina+DPI 畫質與視窗操作的正式方案。

## 功能支援比較

### BlueCG

A6 是目前最完整的 BlueCG workaround：
...
A4 仍能避開黑屏，但高 DPI 會留下黑邊；A2 只在 RetinaMode=`n` 達成拖曳正常；A2 + RetinaMode=`y` 已確認黑屏。
...
| 類型 | 原版 | A2 | A4 | A6 |
|------|------|----|----|----|
| 不依賴 explicit backing-size 的 2D／GDI | 風險最低 | 風險低 | 風險低至中 | 未知 |
| 使用一般 OpenGL context 的遊戲 | 功能完整但可能遇到同類 resize 回歸 | Retina off 可能受影響 | 可能依賴 stale backing size，風險最高 | **未驗證，需逐遊戲回歸** |
| 依賴 DPI-aware backing pixels 的遊戲 | 預期最好 | Retina off 可能尺寸不符 | 可能模糊、裁切或黑邊 | 目前設計最接近預期，但僅 BlueCG 已驗證 |
| 需要 fullscreen／display mode transition | 原始設計 | Retina path 仍有問題 | BlueCG 已通過，但其它遊戲未知 | BlueCG 已通過，其它遊戲未知 |
| 其它遊戲的整體風險 | baseline | 中 | 高 | **未知；目前只對 BlueCG 驗收** |
...
| 面向 | 原版 | A2 | A4 | A6 |
|------|------|----|----|----|
| resize 期間 GL surface rebuild | 反覆發生，可能卡頓／延遲 | 非 Retina 跳過 | 全部跳過 | live resize 延後；same-view 不 teardown |
| resize CPU／GPU 額外工作 | 最高 | 非 Retina 較低 | 最低 | 以一次 final in-place commit 為主 |
| surface 重建造成的瞬時資源 | 可能增加 | 非 Retina 較低 | 最低 | 同一 view 避免重建；view switch 仍保留原流程 |
| 正常遊玩每幀成本 | 理論上正常 | 理論上接近原版 | 理論上接近原版 | 理論上接近原版；尚無 benchmark |
| stale／錯尺寸 drawable 風險 | 黑屏風險 | Retina off 較低 | 由黑屏轉為尺寸／畫質風險 | **BlueCG 已排除；其它遊戲未知** |
...
- 不必接受 A4 的高 DPI 黑邊；仍應以 BlueCG 專用 engine 使用

若只做非 Retina A/B 測試，可使用 A2；若 A6 artifact 無法使用，A4 是 BlueCG 的功能 fallback，但可能有高 DPI 黑邊。

### 開發／除錯

保留 baseline、A2、A4 與 A6 runtime：

```text
baseline：確認原始回歸
A2：確認非 Retina guard 效果
A4：確認完整停用 backing path 的上限效果
A6：確認正式 same-view backing sync 與 restore geometry
```
