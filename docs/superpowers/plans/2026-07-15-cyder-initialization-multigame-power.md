...
7. 省電第一版只提供「標準」與實驗性的「平衡（utility）」；`background` 保留為進階實驗選項，不作預設省電模式。
...
### 5.1 `utility` 或 `background`

目前實測 `background` 畫面很卡，符合其用途；它會降低排程與 I/O 優先級，不適合仍需平順繪製的遊戲。第一個可用原型應選 `utility`：
...
- **標準**：目前行為，適合遊玩。
- **平衡（實驗）**：`taskpolicy -c utility`，適合掛機但仍需查看畫面。
- **最低資源（進階實驗）**：之後才評估 background；明確警告畫面與操作會明顯變慢。

### 5.2 限制與量測

`taskpolicy` 只調整 CPU／I/O 排程，不能保證遊戲降低 FPS 或 GPU 使用率。若遊戲仍無限制重畫，真正耗電來源可能不會顯著下降。後續可研究遊戲內 FPS 限制、renderer present pacing 或最小化行為，但不可在未驗證時宣稱「省電」。
...
若 utility 節能差異不明顯，不應先進正式設定；可保持實驗 flag。
...
**目的**：先取得 utility 是否有實際收益的證據，不急著承諾產品功能。

**範圍**：

- launch service 支援 normal／utility／background 實驗 flag。
- 確保 policy 套在會建立 wineserver 的最外層 process。
- BlueCG 掛機場景量測腳本與報告。
- 記錄 Intel／Apple Silicon 差異、畫面、音訊、網路與計時器回歸。

**驗收**：三模式可重現量測；utility 不造成不可接受卡頓才進 UI；同 bottle session 切換會被阻擋。
...
- utility 只有在量測證明有收益且不破壞掛機功能後，才標示為省電選項。
