...
本輪建議先完成 **A：初始化可靠性**，再建立 **B：每遊戲 Profile／Bottle**，最後才把 **C：省電模式**接入正式 UI。
...
7. 省電模式 UI 統一提供「標準」、「省電」、「極致省電」三個選項；內部分別對應 normal、utility、background，後兩者都必須以量測結果決定是否正式開放。
...
| C 省電 | 啟動固定使用 `/usr/bin/arch -x86_64 wine ...`，未接 `taskpolicy`，也沒有量測與狀態模型。 |
...
### 5.1 「標準／省電／極致省電」的技術對應

目前實測 `background` 畫面很卡，符合其用途；它會降低排程與 I/O 優先級，不適合仍需平順繪製的遊戲。三個 UI 選項建議對應如下：

| UI 選項 | 內部模式 | 啟動方式 | 定位 |
|---|---|---|---|
| 標準 | `normal` | 不包 `taskpolicy` | 一般遊玩，維持目前行為 |
| 省電 | `utility` | `taskpolicy -c utility` | 掛機或低互動情境；需實測確認收益 |
| 極致省電 | `background` | `taskpolicy -c background`，可搭配 background I/O policy | 進階實驗；預期畫面與操作明顯變慢 |

第一個原型仍應先實作並量測 `utility`：

```bash
/usr/bin/taskpolicy -c utility \
...
- **標準**：目前行為，適合遊玩。
- **省電**：`taskpolicy -c utility`，適合掛機但仍需查看畫面；顯示「省電效果依遊戲而異」。
- **極致省電**：`taskpolicy -c background`，明確警告畫面更新、音訊與操作可能明顯變慢，第一版可先標記為實驗功能。

### 5.2 限制與量測

`taskpolicy` 只調整 CPU／I/O 排程，不能保證遊戲降低 FPS 或 GPU 使用率。若遊戲仍無限制重畫，真正耗電來源可能不會顯著下降；因此「省電」與「極致省電」是產品模式名稱，不代表每個遊戲都有固定節能比例。後續可研究遊戲內 FPS 限制、renderer present pacing 或最小化行為，但不可在未驗證時宣稱實際節能數值。

QoS 也與 wineserver 有 session 問題：若 wineserver 已由標準模式啟動，後開的 utility client 不代表既有 server 一併改變。第一版規則與 sync 相同：power mode 在 bottle 無行程時決定，session 存活期間不可切換；切換需關閉該 bottle 全部 EXE 後重開。

原型驗收至少記錄：

- 相同場景 10–15 分鐘的 CPU、Energy Impact、GPU、平均／低百分位 frame time。
- 網路連線、計時器、音訊與掛機功能是否持續。
- 正常、utility、background 三組比較。
- Intel 與 Apple Silicon 各一台；Intel 是本輪主要風險機型。

若 utility 節能差異不明顯，「省電」可保留為實驗選項；若 background 對畫面、音訊或網路掛機造成不可接受影響，「極致省電」應只在進階設定中顯示，或暫不提供。
...
**目的**：先取得「省電」與「極致省電」是否有實際收益及可接受體驗的證據，不急著承諾固定節能比例。

**範圍**：

- launch service 支援 normal／utility／background 三種內部模式，UI 文案固定為「標準／省電／極致省電」。
- 確保 policy 套在會建立 wineserver 的最外層 process。
- BlueCG 掛機場景量測腳本與報告。
- 記錄 Intel／Apple Silicon 差異、畫面、音訊、網路與計時器回歸。

**驗收**：三模式可重現量測；utility 不造成不可接受卡頓才正式顯示「省電」；background 的畫面、音訊、網路掛機結果通過最低門檻才顯示「極致省電」；同 bottle session 切換會被阻擋。
...
- `utility` 只有在量測證明有收益且不破壞掛機功能後，才正式標示為「省電」。
- `background` 必須通過畫面、音訊與掛機穩定性門檻，才標示為「極致省電」；否則保留為進階實驗模式。
