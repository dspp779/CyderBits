...
| T05 Template／Profile backend | 程式完成 | pristine／golden versioned template、固定校驗 Mono 10.4.1／Gecko 2.47.4、Golden→Shared 單向 APFS clone、path-based Profile、staging publish | security-scoped bookmark／EXE 移動後重新綁定 |
| T06 遊戲設定與路由 | 核心完成 | 未綁定 EXE 使用 Shared；偏好設定可從 Golden 建立獨立 bottle；DPI／sync／Retina／字體／能源／env／args 安全逐項傳遞 | bookmark、launcher＋game 多 EXE 共用 Profile、實機 UX 驗收 |
...
| T10 發布驗收 | 進行中 | 主要自動測試、指定 CX26 引擎 Universal app 打包、Apple Silicon 實際建立 Pristine→Golden→Shared 並查核 registry／Mono／Gecko | Intel smoke test、Finder 雙擊與 UI 手動驗收、release notes 與正式發佈簽署 |
...
3. 保留 `bottles/shared` 作為未建立個別設定之 EXE 的預設環境；使用者只有在偏好設定明確建立遊戲 Profile 時，才從 Golden 複製獨立 bottle。Profile 以執行檔標準化路徑識別，檔名只作顯示與 Wine `AppDefaults` 鍵值，不可作唯一 ID。
4. 建立兩層模板：`templates/pristine` 只完成 wineboot 並停用自動 Mono／Gecko 安裝；`templates/golden` 由 pristine clone，安裝固定版本 Mono／Gecko 與共通元件，再套用 Cyder 基準設定。Shared 與個別 Profile 都只能由 Golden 往下複製，禁止由 Shared 反向發布模板。
...
~/Library/Application Support/Cyder/
  templates/
    pristine/<engine-id>/
    golden/<engine-id>-<baseline-revision>/
...
3. `golden`：從 pristine 以 `cp -cR -p` clone，安裝 Mono 10.4.1、Gecko 2.47.4、共通元件與固定基準設定，通過 probe。
4. 從 Golden clone `bottles/shared`；未知 EXE 直接使用 Shared，不在啟動時建立 Profile。
5. 使用者在偏好設定選擇 EXE 後，才從 Golden clone 獨立 Profile bottle；遊戲 recipe 只修改這個獨立 bottle。
...
**目的**：落實 pristine／golden 模板、Shared 預設路徑與按需每遊戲隔離。
...
- Shared 必須由 Golden clone；重建採 staging、backup、health check 與 rollback，禁止 Shared→Golden。
...
- 未建立 Profile 的 EXE 使用 Shared；偏好設定提供「建立獨立遊戲環境」入口，只有明確選擇時才 clone Golden。
