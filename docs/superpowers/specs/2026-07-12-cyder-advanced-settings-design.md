...
- **確認後儲存、下次啟動套用**：控制項先形成草稿，確認時才寫入設定；可選擇以 `wineserver -k` 關閉現有 EXE。
...
1. 開啟 Cyder。
2. 點齒輪、`⌘,` 或 Dock「進階設定…」。
3. 在 sidebar 選擇頁面並調整，底部顯示「有尚未儲存的變更」。
4. 按「確認」，選擇「儲存並關閉所有 EXE」、「僅儲存」或「取消」。
5. 前兩者寫入 `settings.json` 後關閉設定視窗；第一個選項另執行共用 prefix 的 `wineserver -k`。
6. 選擇 `.exe`。
